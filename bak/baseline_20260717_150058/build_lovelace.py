...
def make_ic_css(server_map, fixed_styles_map):
    ...
    IC_CSS += 'ha-card .ig-title-badge{display:inline-flex!important;align-items:baseline!important;font-size:0.65em!important;padding:2px 6px!important;border-radius:6px!important;background:color-mix(in srgb,var(--primary-color) 15%,transparent)!important;color:var(--primary-text-color)!important;line-height:1!important}'
    IC_CSS += 'ha-card .ig-badge-text{position:relative!important;top:1px!important}'
                ...
                for desc in ic_descs:
                    ...
                    if isinstance(desc, dict) and desc.get('type') == 'icon_group':
                        ...
                        if ig_title:
                            ...
                            if _ig_mdi:
                                title_icon_html = '<ha-icon icon="mdi:{}" style="color:inherit;--mdc-icon-size:14px;width:14px;height:14px;margin-right:2px;vertical-align:middle"></ha-icon>'.format(_ig_mdi.group(1))
                                _ig_title_text = ig_title[_ig_mdi.end():].strip()
                            elif linear_maps:
                                sm = _lookup_style(linear_maps[0])
                                icon = sm.get('icon', 'mdi:map')
                                title_icon_html = '<ha-icon icon="{}" style="color:inherit;--mdc-icon-size:14px;width:14px;height:14px;margin-right:2px;vertical-align:middle"></ha-icon>'.format(icon)
                            elif block_maps:
                            ...
                            if block_maps:
                                ...
                                parts.append('<span style="white-space:nowrap;font-size:0.85em">{}{}</span>'.format(title_icon_html, _ig_title_text))
                                parts.append('</div>')
                            else:
                                parts.append('<div class="ig-title-row" style="display:flex;align-items:center;gap:8px;margin:4px 0">')
