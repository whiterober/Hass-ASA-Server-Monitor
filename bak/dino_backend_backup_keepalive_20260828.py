...
def rcon_command(host, port, password, command, timeout=RCON_TIMEOUT):
    ...
    参照 AppDaemon receive_packets：认证/命令各读首个完整包，避免大响应挂起。"""
    s = socket.create_connection((host, port), timeout=timeout)
    try:
        # 认证
        payload = struct.pack('<ii', 1, 3) + password.encode('utf-8') + b'\x00\x00'
        s.sendall(payload)
        auth = _recv_packet(s, timeout)
        if not auth:
            return b''
        ...
        s.sendall(payload2)
        data = _recv_packet(s, timeout)
        if not data:
            return b''
        # 附加包（大响应可能分多个包，最多收满 1MB 或超时）
        extra = b''
        s.settimeout(2.0)
...
_RCON_EXECUTOR = __import__('concurrent.futures', fromlist=['ThreadPoolExecutor']).ThreadPoolExecutor(max_workers=2)
...
def refresh_tamed(server):
    ...
    mem_status('sensor.refresh_tamed_status', 'ok' if updated else 'error', result)
    return result
