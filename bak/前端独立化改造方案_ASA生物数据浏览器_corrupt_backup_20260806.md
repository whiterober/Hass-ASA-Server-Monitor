...
│ 时间降级逻辑（cryoAtMs/tamedAtMs=0 处理）        │
└────────downloadedAtMs 有效性判定） ───────────────┘
                   ▼
┌─ 应用层 ───────────────────────────────────────┐
...
downloadedAtMs` 有效（>0 且 ≥ 2023-10-25）且 `now - downloadedAtMs ≤ 24h`
...
- **判定**：
  - 每条记录 → `ownerTribeId`（见 5.3）∈ 部落集合 且 `atMs > 0` 且 `now - atMs ≤ 24h`
  - cryo 用 `cryoAtMs`（封球时间=新增入球）、tamed 用 `tamedAtMs`（驯服时间）
- **展示**：按服务器分组 → 每条显示 名称/等级/性别/来源(球内/放出)/容器名/封球或驯服时间（本地时间）
- `atMs=0` 的记录：单独"时间未知"折叠区，提示"旧球无封球时间，重新封球后可见"
...
---downloadedAtMs 覆盖**：确认 v6 旧球/宠物台是否会补下载时间戳（当前仅 v7 新格式球带此字段

## 8. 风险与待确认清单

- [ ] **wiim web 根与认证**：401 原因（正确凭据/是否需登录态）；web 根是否 `P:\wikily-import`
- [ ] **DinoData 前端可达路径**：选 7.2 的 A/B/C
- [ ] **cryoAtMs=0 回填**：确认插件/读档端是否会回填旧球时间（或仅新封球有值）；前端降级逻辑已覆盖
