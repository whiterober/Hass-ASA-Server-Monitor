        ...
        for block in tab.get('content_blocks',[]):
            ...
            if bt=='text':
                ...
                for desc in ic_descs:
                    ...
                    dserver_attr=' data-server="{}"'.format(dserver) if dserver else ''
                ...
                filter_js=(
                    ...
                    "r.querySelectorAll('.filterable,.sc-srv,.ic-text[data-server]').forEach(function(e){e.style.setProperty('display','','')});"
                    "return}"
    ...
    def make_content_card(html_content, tab_type=None, tab=None):
        if tab_type == 'mixed_content' and tab:
            ...
            if 'supply_card' in block_types or 'map_filter' in block_types:
                ...
                css += 'ha-card:has(.filter-radio:checked) .filterable[data-filter-maps=\"\"]{display:flex!important}'
