...
- [ ] **数据层**：10 图 `facilities[]` 条数与实测一致（Isl 1437 / Abe 793 / Ast 1214 …）
- [ ] **公共设施**：抽样 3 图 126 条 `isPublic=1` ⇒ 按选定口径处理（未污染聚落/区域）
- [ ] **缓存**：`LB_SCHEMA_VER` 升级后，**旧用户强刷即见新家族**（不清缓存也能自动重建）
- [ ] **汉化**：新家族 chip 全中文、无英文类名裸露（重点抽查耕地 `_SM`、电力节点、城市终端）
- [ ] **图标**：`LB_ICON_BY_NAME` 命中验证（未命中的走 MDI 齿轮，需确认可接受）
- [ ] **区域算法**：改动前后区域数量 / 命名对照（Isl 与 Abe 必做）
- [ ] **回归**：容器手风琴无重复项（facility 与容器 rec 同源）；床/牌/碑改名入口不受影响
- [ ] **性能**：首屏 CJK 无卡顿（Isl recs 体量增长后 `lbBuildSettlements` 耗时对比）
