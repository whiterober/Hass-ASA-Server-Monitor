...
def main():
    ...
    # Handle base_quick_ref format: nested servers structure
    if source == 'base_quick_ref' or source.startswith('base_quick_ref/'):
        parts = source.split('/')
        server_id = parts[1] if len(parts) > 1 else 'Isl'
        servers = data.get('servers', {})
        srv = servers.get(server_id, {})
        tabs = srv.get('tabs', [])
        if not tabs:
            print("ERROR: no tabs for server", server_id)
            sys.exit(1)
        tab = None
        try:
...
// Accordion (scheme 2): expand/collapse with smooth animation
window.toggleAcc=function(s){var a=s.parentElement;var g=a.getAttribute('data-acc-group')||'ic-acc';if(a.classList.contains('collapsed')){if(g){document.querySelectorAll('[data-acc-group='+g+']').forEach(function(x){if(x!==a&&!x.classList.contains('collapsed'))window.collapseAcc(x)})}window.expandAcc(a)}};
window.expandAcc=function(a){a.classList.remove('collapsed');a.style.maxHeight='0px';a.offsetHeight;a.style.maxHeight=(a.scrollHeight+20)+'px';setTimeout(function(){a.style.maxHeight='none'},400)};
window.collapseAcc=function(a){a.style.maxHeight=a.scrollHeight+'px';requestAnimationFrame(function(){a.classList.add('collapsed');a.style.maxHeight=''})};

// Card-level click to expand: click anywhere on a card with collapsed fold
document.addEventListener('click',function(e){
  if(e.target.closest('.ic-acc-arrow,.ic-copy-key,button,a,ha-icon')) return;
  var card = e.target.closest('.info-card-block');
  if(!card) return;
  var acc = card.querySelector('.ic-acc.collapsed');
