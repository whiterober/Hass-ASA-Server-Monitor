...
| `anyEventActive` | 0/1 | 是否有事件正在进行（1 时建议显示“事件中”，隐藏倒计时）。⚠️ 实测**只有部分轮次会翻到 1**（真喷发轮次）；多数轮次只有地震（`warmUpActive`），此时保持 `0` 属正常 |
...
| `hooks` | object | `activate` / `deactivate`（1 = 钩子已挂上，**诊断用**：实测 `AHazardTrigger::Activate/Deactivate` 从不参与喷发循环）、`inEvent`（恒 0）。**⭐ `events` = 台账累计事件数（单调递增，服务器重启归零）——这是最可靠的“是否有新事件”判据**（见 §4.1） |
...
- `AnyEventActive` `0↔1` → `activeOn` / `activeOff`（**只在真喷发轮次出现**：实测 2026-09-14 20:14~22:00 共 9 轮，仅 3 轮翻到 1）

**推荐的增量拉取方式**

- **首选判据**：比对 `hooks.events`（台账累计总数）—— 比上次大 ⇒ 期间发生了新事件，差值即新增条数。该值**单调递增、不受 64 条环形缓冲截断**
- 备选：比对 `history[]` 末尾 `worldSec`（注意环形缓冲只留最近 64 条）
- ⚠️ 服务器重启 / 插件重载后 `hooks.events` **归零**，前端需重置本地基准（否则会把重启当成“事件消失”）
- 台账精度为 **±1 s**（1 Hz 采样），帧级精确不可得：`AHazardTrigger::Activate/Deactivate` 钩子已实服证伪（挂得上，但 36 条实服日志中无一条 `VOLCANO_EVENT_BEGIN class=` 格式）
...
- **两种轮次**（实测 2026-09-14 20:14~22:00，共 9 轮）：
  - **只地震**（多数，9 轮里 6 轮）：`warmupOn → warmupOff`，`anyEventActive` 全程 `0`，`phase` 走 `warmup → cooldown`
  - **真喷发**（少数，9 轮里 3 轮）：额外出现 `activeOn → activeOff`（`AnyEventActive` 翻到 `1`），`phase` 会经过 `eruption`
...
> ⚠️ `eruption` 阶段**不常见**：实测多数轮次只走 `warmup → cooldown`（`anyEventActive` 保持 `0`），只有“真喷发轮次”才会出现 `eruption`。前端**不要**因为长时间看不到 `eruption` 就判定异常。
...
6. `hooks.activate=1` / `hooks.deactivate=1` → 钩子已挂上（**但这两个钩子从不触发，属已知现象；事件计数请看 `hooks.events`**）✅
