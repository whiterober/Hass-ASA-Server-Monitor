...
### ✅ 4.4-实测判定（2026-09-20）：**P1-① 无需改动**（先量后改的结论）

**量化方式**：线下近似复刻前端几何链（分层 `tol=300` → 层内 `DBSCAN(eps=1800, minPts=2)` → 噪声并入最近簇），
数据取实服 `facilities[]`（排除 `isPublic=1`、保留有 `tribeId`、排除标牌类、按 `cls|坐标` 去重），**按聚落分别建区**；
对比「含耕地」与「排除耕地」两种情形的区域总数（脚本 `tmp/_farm_geom.py`，产物 `tmp/_farmgeom.txt`）。

| 图 | 聚落(tribeId) | 设施(含耕) | 区域(含耕) | 区域(排耕) | Δ | 耕地数 |
|---|---|---:|---:|---:|---:|---:|
| Isl | 19125947 | 963 | **88** | 87 | +1 | 79 |
| Isl | 16213691 | 416 | 43 | 43 | 0 | 30 |
| Abe | 18919496 | 331 | **60** | 58 | +2 | 39 |
| Abe | 16046851 | 187 | 37 | 37 | 0 | 8 |
| Abe | 10541618 | 161 | 17 | 17 | 0 | 6 |
| 其余 11 聚落 | — | ≤16 | 各 ≤6 | 同 | 0 | ≤5 |

**汇总**：区域总数 **266（含耕） vs 263（排耕）⇒ 仅 +3 个（+1.1%）**；167 块耕地里绝大多数落在**已有区域内**，
只有 3 处形成新区域 ⇒ **耕地并未造成区域碎裂/形状漂移**，方案 A（不进分区）与方案 B（按类型合并）**均无必要**。

**显示侧已自带聚合（源码事实）**：`_mkAreaChips` 中**无名设施按类型 key 入 `_grp` 并 `n++`** ⇒
耕地在区域内自动显示为「小型耕地 ×63」这类**单条 ×N chip**（用户实测「耕地 chip 已展示」即此形态）；
仅**带自定义名**或**带涂装色点**的条目才独立成 chip（少数）。

**判定**：P1-① **关闭**（保持现状），不实施 A/B；若后续实服出现"某聚落耕地把区域撑碎"的个例，再按 A/B 局部处理。
