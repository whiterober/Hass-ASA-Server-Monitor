...
class ASAServerMonitorReliable(hass.Hass):
    def initialize(self):
        ...
        # 最新生物：监听 HA 事件触发 tamed 刷新（前端 script → event → listen_event）
        # 前端调 script.refresh_tamed → HA 发事件 refresh_tamed_request → 本回调 RCON 触发
        self.listen_event(self.refresh_tamed_event, "refresh_tamed_request")

        # 延迟开始监控
        self.run_in(self.start_monitoring, 3)

    def _write_refresh_status_file(self, state, attrs):
        """写免认证静态 JSON（前端 webhook 模式无 token 读取）：/config/www/asa-data/refresh_status.json
        通过现有 shell_command.save_asa_data 写入（python3 save_data.py，AppDaemon 容器写不到 HA /config/www/）。
        """
        try:
            ...
            self.call_service("shell_command/save_asa_data", filename="refresh_status.json", b64data=b64)
        except Exception as e:
            self.log(f"[refresh_tamed] write status file failed: {e}", level="WARNING")

    def refresh_tamed_event(self, event_name, data, kwargs):
        """最新生物：触发指定服务器 tamed sidecar 重写（RCON ArkTamedDinos）。
        由前端「最新生物」Tab → HA script.refresh_tamed / webhook → 事件 refresh_tamed_request → 本回调。
        RCON 结果（含 cooldown）写入 sensor.refresh_tamed_status 供前端读取判断。
        同时写免认证静态 JSON（/config/www/asa-data/refresh_status.json）供手机 App（无 token）读取。
        """
        status_entity = "sensor.refresh_tamed_status"
        server = (data or {}).get("server", "").strip()
        if not server:
            self.log("[refresh_tamed] missing server param", level="WARNING")
            attrs = {"server": "", "ok": False, "cooldown": False, "error": "missing server"}
            self.set_state(status_entity, state="error", attributes=attrs)
            self._write_refresh_status_file("error", attrs)
            return
        port = self.rcon_ports.get(server)
        if not port:
            self.log(f"[refresh_tamed] no RCON port configured for {server}", level="WARNING")
            attrs = {"server": server, "ok": False, "cooldown": False, "error": "no rcon port"}
            self.set_state(status_entity, state="error", attributes=attrs)
            self._write_refresh_status_file("error", attrs)
            return
        try:
            success, result = send_rcon_command_sync(
                ...
                "TransferIdentityFix.ArkTamedDinos"
            )
            ok = False
            cooldown = False
            if success and result:
                try:
                    ...
                    ok = True  # 非 JSON 响应按成功处理
            attrs = {"server": server, "ok": ok, "cooldown": cooldown,
                     "result": str(result)[:200], "ts": str(datetime.now())}
            self.set_state(status_entity, state="ok" if ok else ("cooldown" if cooldown else "error"),
                           attributes=attrs)
            self._write_refresh_status_file("ok" if ok else ("cooldown" if cooldown else "error"), attrs)
            self.log(f"[refresh_tamed] {server} -> success={success} ok={ok} cooldown={cooldown} result={str(result)[:200]}")
        except Exception as e:
            attrs = {"server": server, "ok": False, "cooldown": False, "error": str(e)}
            self.set_state(status_entity, state="error", attributes=attrs)
            self._write_refresh_status_file("error", attrs)
            self.log(f"[refresh_tamed] {server} error: {e}", level="ERROR")
