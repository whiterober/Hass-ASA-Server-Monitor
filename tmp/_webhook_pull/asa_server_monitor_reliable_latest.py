...
class ASAServerMonitorReliable(hass.Hass):
    ...
    def refresh_tamed_event(self, event_name, data, kwargs):
        ...
        由前端「最新生物」Tab → HA script.refresh_tamed → 事件 refresh_tamed_request → 本回调。
        ...
        if not server:
            ...
            self.set_state(status_entity, state="error",
                           attributes={"server": "", "ok": False, "cooldown": False, "error": "missing server"})
            return
        port = self.rcon_ports.get(server)
        if not port:
            self.log(f"[refresh_tamed] no RCON port configured for {server}", level="WARNING")
            self.set_state(status_entity, state="error",
                           attributes={"server": server, "ok": False, "cooldown": False, "error": "no rcon port"})
            return
        try:
            ...
            self.set_state(status_entity, state="ok" if ok else ("cooldown" if cooldown else "error"),
                           attributes={"server": server, "ok": ok, "cooldown": cooldown,
            ...
            self.set_state(status_entity, state="error",
                           attributes={"server": server, "ok": False, "cooldown": False, "error": str(e)})
