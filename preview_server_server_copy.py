...
def main():
    ...
    if ttype == 'mixed_content':
        ...
        if 'supply_card' in block_types or 'filtered_cards' in block_types:
            css += 'ha-card .supply-card{border-radius:12px!important;border:1px solid var(--divider-color)!important;margin-bottom:8px!important;padding:12px!important;display:flex!important;gap:12px!important;background:var(--primary-background-color)!important}'
            ...
            css += 'ha-card .supply-card .sc-body .sc-srv.off{opacity:.35!important}'
...
</head><body style="background:var(--primary-background-color);color:var(--primary-text-color);padding:10px;margin:0">
<ha-card>%s</ha-card>
<script>
